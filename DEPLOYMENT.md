...
Vercel's deployed filesystem is read-only. The server automatically uses `/tmp`
there so it can respond instead of crashing, but `/tmp` is temporary. For
permanent financial records, deploy the API to a host with a persistent volume
and keep the data directory mounted.
